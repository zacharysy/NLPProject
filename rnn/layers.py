...
class Embedding(torch.nn.Module):
    ...
    def __init__(self, vocab_size, output_dims):
        ...
        self.W = torch.nn.Parameter(torch.empty(
            vocab_size, output_dims, device=get_device()))
...
class RNN(torch.nn.Module):
    ...
    def __init__(self, dims):
        super().__init__()
        self.h0 = torch.nn.Parameter(torch.empty(dims, device=get_device()))
        self.W_hi = torch.nn.Parameter(
            torch.empty(dims, dims, device=get_device()))
        self.W_hh = torch.nn.Parameter(
            torch.empty(dims, dims, device=get_device()))
        self.b = torch.nn.Parameter(torch.empty(dims, device=get_device()))
    ...
    def input(self, state, inp):
        ...
        if h_prev.size()[-1] != dims:
            raise TypeError(
                f'Previous hidden-state vector must have size {dims}')
        if inp.size()[-1] != dims:
            raise TypeError(f'Input vector must have size {dims}')

        h_cur = torch.tanh(bmv(self.W_hi, inp) +
                           bmv(self.W_hh, h_prev) + self.b)
        return (inp, h_cur)
...
class TanhLayer(torch.nn.Module):
    ...
    def __init__(self, input_dims, output_dims, residual=False):
        ...
        if residual and input_dims != output_dims:
            raise ValueError(
                "A residual connection requires the same number of input and output dimensions.")
        self.W = torch.nn.Parameter(torch.empty(
            output_dims, input_dims, device=get_device()))
        self.b = torch.nn.Parameter(torch.empty(
            output_dims, device=get_device()))
        torch.nn.init.normal_(self.W, std=0.01)
        torch.nn.init.normal_(self.b, std=0.01)
    ...
    def forward(self, inp):
        ...
        *or*
        ...
        if inp.size()[-1] != input_dims:
            raise TypeError(
                f"The inputs must have size {input_dims} (not {inp.size()[-1]})")
...
def attention(query, keys, vals):
    ...
    *or*
    ...
    if query.size()[-1] != keys.size()[-1]:
        raise TypeError(
            "The queries and keys should be the same size (last axis)")
    if keys.size()[-2] != vals.size()[-2]:
        raise TypeError(
            "There must be the same number of keys and values (second-to-last axis)")
...
class SelfAttention(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        if inputs.size()[-1] != dims:
            raise TypeError(
                f"input vectors must have size {dims}, not {inputs.size()[-1]}")
...
class MaskedSelfAttention(torch.nn.Module):
    ...
    def __init__(self, dims):
        ...
        self.W_V = torch.nn.Parameter(
            torch.empty(dims, dims, device=get_device()))
        self.initial = torch.nn.Parameter(
            torch.empty(dims, device=get_device()))
        torch.nn.init.normal_(self.W_Q, std=0.01)
        torch.nn.init.normal_(self.W_K, std=0.01)
        torch.nn.init.normal_(self.W_V, std=0.01)
    ...
    def input(self, prev_inps, inp):
        ...
        inputs = torch.cat([prev_inps.cuda(), inp.unsqueeze(0).cuda()], dim=0)
        return inputs
